/** Client-side mirror of backend calendar-oauth-errors (defense in depth for URL params). */
...
export function outlookErrorTitle(message: string): string {
  const lower = message.toLowerCase();
  if (lower.includes("wasn't granted") || lower.includes("permission")) return "Permission needed";
  if (lower.includes("cancelled")) return "Connection cancelled";
  if (lower.includes("contact support")) return "Setup issue";
  if (lower.includes("session expired") || lower.includes("disconnect")) return "Session expired";
  if (lower.includes("refresh") || lower.includes("sync")) return "Sync issue";
  return "Couldn't connect Outlook";
}
